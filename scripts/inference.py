...
def inference(model, img, device):
    ...
    img = img.div(255).sub(0.5).div(0.5).to(device)
...
def process_dataset(cfg, weight, img_dir, output_file):
    ...
    img_files = [img_name for img_name in os.listdir(img_dir) if img_name.endswith(".jpg")]

    for img_name in tqdm(img_files, desc="Processing images"):
        img_path = os.path.join(img_dir, img_name)
        output = inference(model, img_path, device)
        result = {"image": img_name}

        for each in output.keys():
            if each not in exclude_attributes:
                values = output[each][0].cpu().numpy()
